import os
from analyzer import Analyzer, plot_vectors2, plot_vectors
...
def example():
    ...
    plot_vectors2(tf_idf_vector_list, author_list)
    plot_vectors(w2v_vector_list, author_list)
    plot_vectors2(w2v_vector_list, author_list)
    ...
    print("**基于词向量的分析")
    for word in ["春","梅","归","静","忧","愁","枕","鸳鸯","垂柳"]:
        print("与 %s 相关的词：" % word)
        print_counter(analyzer.find_similar_word(word))

    for poet in ["李白", "杜甫", "白居易", "李贺"]:
